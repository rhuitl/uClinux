...
char **getGETvars() {
    ...
    getinput=strdup(getenv("QUERY_STRING"));

    /* Change all plusses back to spaces */
    for(i=0; getinput[i]; i++) if (getinput[i] == '+') getinput[i] = ' ';

    /* First, split on "&" to extract the name-value pairs into pairlist */
    pairlist=(char **) malloc(256*sizeof(char **));
    paircount=0;
    nvpair=strtok(getinput, "&");
    while(nvpair) {
    ...
    getvars[paircount*2]=0; /* terminate the list with NULL */
...
char **getPOSTvars() {
    ...
    for(i=0; postinput[i]; i++) if (postinput[i] == '+') postinput[i] = ' ';
