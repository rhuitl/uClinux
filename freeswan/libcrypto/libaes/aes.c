...
#else
#ifdef __i386__
/* If the asm version isn't used these settings seem best for the SC520 */
#define UNROLL
#define FOUR_TABLES
#define ONE_LR_TABLES
#define FOUR_IM_TABLES
#else
#ifdef __mcf5200__
/* Coldfire processors have a few problems.  Code alignment seems to make
 * a difference :(  These settings are for configurations that seem stable
 * with respect to several compilations and which are fastest some of the
 * time (i.e. fastest sometimes and always good).
 */
/* How to pick CPU type?  This should work but isn't nice */
...
#define const_word_in(x)      ((const u_int32_t)(((unsigned char *)(x))[0])|((const u_int32_t)(((unsigned char *)(x))[1])<<8)|((const u_int32_t)(((unsigned char *)(x))[2])<<16)|((const u_int32_t)(((unsigned char *)(x))[3])<<24))
