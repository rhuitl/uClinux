...
char ipcomp_c_version[] = "RCSID $Id: ipcomp.c,v 1.28 2002/01/29 17:17:54 mcr Exp $";
...
extern unsigned int	(*hw_lzs_decompress)(unsigned char *in, int ilen,
				unsigned char *out, int olen);
extern unsigned int	lzs_compress(unsigned char *in, int ilen,
				unsigned char *out, int olen);
extern unsigned int	lzs_decompress(unsigned char *in, int ilen,
				unsigned char *out, int olen);
...
struct sk_buff *skb_compress(struct sk_buff *skb, struct ipsec_sa *tdb, unsigned int *flags)
{
	...
	if (tdb->tdb_encalg != SADB_X_CALG_LZS) {
	...
	if (tdb->tdb_encalg == SADB_X_CALG_LZS) {
		...
		inbuf = (char *) iph + iphlen; /* start of payload */
		if ((rc = _lzs_compress(inbuf, pyldsz, buffer, cpyldsz)) < 0)
			goto badcompress;
		cpyldsz = rc;
	} else {
#endif /* CONFIG_IPSEC_IPCOMP_LZS */
...
struct sk_buff *skb_decompress(struct sk_buff *skb, struct ipsec_sa *tdb, unsigned int *flags)
{
	...
	unsigned int iphlen, pyldsz, cpyldsz, avail_out;
	z_stream zs;
	int zresult;
	...
	if ( (((struct ipcomphdr*)((char*) oiph + iphlen))->ipcomp_flags != 0)
		...
		!= htons(SADB_X_CALG_DEFLATE))
		 ...
		 && (!tdb || (tdb && 
		   ((tdb->tdb_encalg != SADB_X_CALG_DEFLATE)
		   && (tdb->tdb_encalg != SADB_X_CALG_LZS)) ))) ) {
		KLIPS_PRINT(sysctl_ipsec_debug_ipcomp,
			    "klips_error:skb_decompress: "
	...
	*/
	...
	if (tdb->tdb_encalg == SADB_X_CALG_LZS) {
		...
		inbuf = (char *) oiph + iphlen + sizeof(struct ipcomphdr);
		outbuf = (char *)iph + iphlen;

		if ((rc = _lzs_decompress(inbuf, cpyldsz, outbuf, pyldsz)) < 0) {
			KLIPS_PRINT(sysctl_ipsec_debug_ipcomp,
				    "klips_error:skb_decompress: "
				    "lzs_decompress() returned error %d, "
				    "skipping decompression.\n", rc);
			*flags |= IPCOMP_DECOMPRESSIONERROR;
#ifdef NET_21
			kfree_skb(nskb);
