...
#define MAX_XMIT_TASK_IN_SOFTIRQ 384
        ...
        case IX_CRYPTO_ACC_STATUS_SUCCESS:
            ...
            else
            {
                ...
                if (pXmitDesc)
                {
                    ...
                    {
                        spin_lock (&tdb_lock);
			delXmitDesc_from_salist(pXmitDesc->tdbp, pXmitDesc);
...
static void ipsec_tunnel_next_transform(void *data)
{
   ...
   /* IXP425 glue code */
    ...
    delXmitDesc_from_salist(tdbp, pXmitDesc);
	...
	tailroom += ((tdbp->ips_enc_blksize - ((pyldsz + 2 * sizeof(unsigned char)) 
    % tdbp->ips_enc_blksize)) % tdbp->ips_enc_blksize) + 2;
	tailroom += authlen;
    ...
    spin_unlock(&tdb_lock);
		...
		while (tdbp) {
			...
			spin_lock (&tdb_lock);
			addXmitDesc_to_salist(tdbp, pXmitDesc);
			iphlen = iph->ihl << 2;
			...
			case IPPROTO_ESP:
				...
				case ESP_3DES:
					headroom += (sizeof(struct esp) + tdbp->ips_iv_size);
					break;
			...
			case SPI_TRAPSUBNET:
			{
				...
				if (pfkey_acquire(&(pXmitDesc->tdb)) == 0) {
					...
					if ((pXmitDesc->outgoing_said).spi==htonl(SPI_TRAPSUBNET)) {
						...
						hold_eroute.er_pid = pXmitDesc->eroute_pid;
...
bypass_cb:
    ...
    KLIPS_PRINT(debug_tunnel & DB_TN_XMIT,
            "klips_debug:ipsec_tunnel_start_xmit: "
            "...done, 2 calling ip_send() on device:%s\n",
...
/*	struct device *virtdev; */
    ...
    /* Get xmit desc */
    if (ipsec_glue_xmit_desc_get(&pXmitDesc) != IPSEC_GLUE_STATUS_SUCCESS)
    {
        if(skb) {
#ifdef NET_21
            kfree_skb(skb);
	...
	if ((natt_type) && ((pXmitDesc->outgoing_said).proto != IPPROTO_IPIP)) {
		...
		ll_headroom = (pXmitDesc->hard_header_len + 15) & ~15;
KLIPS_PRINT(debug_tunnel & DB_TN_CROUT,"A\n");
		if ((skb_headroom(skb) >= max_headroom + 2 * ll_headroom) &&
		    (skb_tailroom(skb) >= max_tailroom)
#ifndef NET_21
			&& skb->free
		...
		while (tdbp) {
			...
			case IPPROTO_ESP:
				...
				tailroom += ((tdbp->ips_enc_blksize - ((pyldsz + 2 * sizeof(unsigned char)) % tdbp->ips_enc_blksize)) 
                            % tdbp->ips_enc_blksize) + 2;
				tailroom += authlen;
				break;
#endif /* !CONFIG_IPSEC_ESP */
#ifdef CONFIG_IPSEC_IPIP
