 ...
 * for more details.
 *
 * RCSID $Id: rsasigkey.c,v 1.20 2002/03/08 00:46:57 henry Exp $
 */
...
int devicechanged = 0;		/* did user supply his own device? */
...
char cop[] = "See `ipsec --copyright' for copyright information.";
...
int useoldkey;			/* take primes from old key? */
{
	...
	if (!success) {			/* e has no inverse mod m (!) */
		fprintf(stderr, "%s: fatal error: unable to invert public "
							"exponent!\n", me);
		if (devicechanged)
			fprintf(stderr, "\t(perhaps random bits are not very "
								"random?)\n");
		exit(1);
	}
	if (mpz_cmp_ui(d, 0) < 0)
	...
	mpz_init(coeff);
	success = mpz_invert(coeff, q, p);	/* coeff = q^-1 mod p */
	if (!success) {			/* q has no inverse mod p (!) */
		fprintf(stderr, "%s: fatal error: unable to invert second "
							"prime!\n", me);
		if (devicechanged)
			fprintf(stderr, "\t(perhaps random bits are not very "
								"random?)\n");
	...
	printf("\t#IN KEY 0x4200 4 1 %s\n",
			conv(bundp, bs, 's')+2);	/* RFC2537 */
	printf("\t# (0x4200 = auth-only host-level, 4 = IPSec, 1 = RSA)\n");
	printf("\tModulus: %s\n", hexout(n));
...
size_t *sizep;
{
	...
	if (size > sizeof(bundbuf)-2) {
		fprintf(stderr, "%s: can't-happen bundle overflow (need %u)\n",
								me, size);
		exit(1);
	}
	if (sizep != NULL)
...
int format;			/* datatot() code */
{
	...
	if (n > sizeof(convbuf)) {
		fprintf(stderr, "%s: can't-happen convert overflow (need %u)\n",
								me, n);
		exit(1);
	}
	return convbuf;
