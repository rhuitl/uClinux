...
#ifndef MIB_IF_ADMIN_STATUS_UP
#define MIB_IF_ADMIN_STATUS_UP 1
#endif
#ifndef MIB_IF_ADMIN_STATUS_DOWN
...
static int writeIfEntry(int action, u_char * var_val, u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len);
...
/* NOTE: This is the version used for linux */
u_char         *
var_ifEntry(struct variable *vp,
            oid * name,
            size_t * length,
    ...
    while (fgets(line, sizeof(line), devin)) {
        ...
        *ptr = 0;
        ...
        if (if_ptr) {
            ...
            /*Zero speed means link problem (but only for ethernet) */
            if(nnew->if_type == 6 && nnew->if_speed == 0 && nnew->if_flags & IFF_UP){
...
static int
if_set_flag(char *ifname, int flag)
{
    struct ifreq ifr;
...
static int
if_clr_flag(char *ifname, int flag)
{
    struct ifreq ifr;
...
static int
set_admin_status(int index, int status)
{
    struct ifnet ifnet;
    struct in_ifaddr ifaddr;
    char if_name[100];

    if (Interface_Scan_By_Index(index, if_name, &ifnet, &ifaddr) == 0) {
        if (status == MIB_IF_ADMIN_STATUS_DOWN) {
            return(if_clr_flag(if_name, IFF_UP));
        }
        else {
            return(if_set_flag(if_name, (IFF_UP | IFF_RUNNING)));
        }
    }
    else {
        return(-1);
             ...
             u_char * statP, oid * name, size_t name_len)
{
    ...
    case COMMIT:               /* Confirm the SET, performing any irreversible actions,
                                 * and free resources */
        if (set_admin_status((int)name[10], *(int *)var_val) != 0) {
            snmp_log(LOG_ERR,
                     "Error in writeIfEntry case COMMIT with index: %d & adminStatus %d\n",
                     (int)name[10], *(int *)var_val);
