#ifndef lint
static char rcsid[] = "$Id: snefru.c,v 1.9 1994/07/25 15:46:52 gkim Exp $";
#endif

/*
 * snefru.c
 * 
 *	signature function hook for Snefru (the Xerox Secure Hash Function) 
 *	for Tripwire.
 *
 *	Originally released by Xerox into the public domain.  Slightly
 *	changed to facilitate hook into other packages and increase
 *	portability.
 *
 *	The original distribution banner follows.
 *
 * Gene Kim
 * Purdue University
 * September 27, 1992
...
#if defined(TW_TYPE32)
typedef unsigned TW_TYPE32 word32;
#else
	...
	FILE *fp;
{
	...
	byteCount = fread(charBuf, 1, CHUNK_SIZE*4, fp);
	if (byteCount < 0)
 ...
 */
int
sig_snefru_get(fd_in, ps_signature, siglen) 
    int fd_in; 
    char *ps_signature;
    int siglen;
{
	...
	do {
		/* Get the next chunk */
		byteCount = ReadChunk (&hash[OUTPUT_BLOCK_SIZE], fp);
	...
	/* base 64 */
	    ...
	    pltob64(hash, ps_signature, OUTPUT_BLOCK_SIZE);
	}
	/*
	for (i = 0; i < OUTPUT_BLOCK_SIZE; i++)
		printf (" %08x", hash[i]);
