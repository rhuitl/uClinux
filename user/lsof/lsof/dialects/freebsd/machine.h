...
/*
 * $Id: machine.h,v 1.36 2006/03/27 23:29:03 abe Exp $
 */
...
/*
 * In FreeBSD >= 5.2 when the cpumask_t typedef is present, it is defined int
 * <sys/types.h> only if _KERNEL is predefined.  However, predefining _KERNEL
 * before #include'ing <sys/types.h> causes redefinition errors for boolean_t
 * and vm_page_t when <vm/vm.h> is #include'd with _KERNEL predefined.  Since
 * lsof must have _KERNEL predefined when <vm/vm.h> is #include'd, the
 * expedient choice is made to duplicate the cpumask_t typedef here.
 *
 * Note: the Configure script defines HASCPUMASK_T if <sys/types.h> and
 * <machine/_types.h> both have cpumask_t references.
 */
