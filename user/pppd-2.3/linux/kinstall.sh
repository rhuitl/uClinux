...
installfile () {
  BASE=`basename $1`
#
# SHA-1 (SHS) sources located in ../pppd/ directory
#
  case $BASE in
    sha*) BASE="../pppd/"$BASE;;
  esac
...
echo
#echo -n 'Adding LZS compression module to drivers makefile...'
#NETMK=$LINUXSRC/drivers/net/Makefile
#fgrep ppp_mppe.o $NETMK >/dev/null
#if [ ! "$?" = "0" ]; then
#   if [ -f $NETMK.orig ]; then
#      mv $NETMK.orig $NETMK
#   fi
#   sed 's/bsd_comp.o$/bsd_comp.o ppp_lzscomp.o/g' <$NETMK >$NETMK.temp
#   bombiffailed
#   echo -n '.'
#   mv $NETMK $NETMK.orig
#   bombiffailed
