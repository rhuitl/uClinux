...
/* This function is called to (re)create the inotify watches */
static int setup_inotify(int existing_inotify, struct udev *udev) {
	int inotify_fd = existing_inotify;
	if (inotify_fd < 0)
	...
	if (inotify_fd >= 0) {
		if (udev_get_rules_path(udev) != NULL) {
			...
			inotify_add_watch(inotify_fd, SYSCONFDIR "/config/udev.d",
					  IN_CREATE | IN_DELETE | IN_MOVE | IN_CLOSE_WRITE);
...
int main(int argc, char *argv[])
{
	...
	inotify_fd = -1;
	inotify_fd = setup_inotify(inotify_fd, udev);
	if (inotify_fd < 0) {
		if (errno == ENOSYS)
