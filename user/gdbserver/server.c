...
{
#ifdef EMBED
  /* We also allow attaching directly to a process */
  if (argv[0][0] == '-' && argv[0][1] == 'p') {
    if (argv[0][2] == '\0')
      inferior_pid = atoi(argv[1]);
    else
      inferior_pid = atoi(argv[0]+2);
    if (inferior_pid <= 0) {
      fprintf(stderr, "No process ID specified\n");
      exit(1);
    }
    attach_inferior(inferior_pid);
    fprintf (stderr, "Attached to process %d\n", inferior_pid);
  } else {
#endif
