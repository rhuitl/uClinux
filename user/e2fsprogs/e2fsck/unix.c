...
static void signal_alarm(int sig)
{
	...
	fprintf(stderr, _("Fsck taking too long, giving up\n"));
	if (ctx != NULL && (ctx->options & E2F_OPT_REFORMAT)) {
		fprintf(stderr, _("Reformatting file system %s\n"), ctx->device_name);
		execl("/bin/mke2fs", "mke2fs", "-j", ctx->device_name, NULL);
	}
	exit(1);
}
#endif
...
static errcode_t PRS(int argc, char *argv[], e2fsck_t *ret_ctx)
{
	...
	*ret_ctx = ctx;
	...
	while ((c = getopt (argc, argv, "panyrcC:B:dE:fvtFVM:b:I:j:P:l:L:N:SsDkT:R")) != EOF)
		switch (c) {
#ifdef HAVE_SIGNAL_H
		case 'R':
			ctx->options |= E2F_OPT_REFORMAT;
			break;
#endif
		case 'T':
			giveup_timeout = atoi(optarg);
			if (giveup_timeout <= 0)
				giveup_timeout = 600;
			break;
		case 'C':
