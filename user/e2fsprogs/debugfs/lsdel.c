...
void do_lsdel(int argc, char **argv)
{
	...
	struct deleted_info	*delarray, *dap;
	...
	while (ino) {
		...
		if (lsd.free_blocks && !lsd.bad_blocks) {
			...
			dap = &delarray[num_delarray];
			dap->ino = ino;
			dap->mode = inode.i_mode;
			dap->uid = inode.i_uid;
			dap->size = inode.i_size;
			if (!LINUX_S_ISDIR(inode.i_mode))
				dap->size |= 
					((__u64) inode.i_size_high << 32);
			dap->dtime = inode.i_dtime;
			dap->num_blocks = lsd.num_blocks;
			dap->free_blocks = lsd.free_blocks;
			num_delarray++;
		}
