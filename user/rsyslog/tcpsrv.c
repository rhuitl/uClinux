...
configureTCPListen(tcpsrv_t *pThis, char *cOptarg)
{
	register int i;
	register char *pArg;
	register char *pNode = NULL;

	assert(cOptarg != NULL);
	ISOBJ_TYPE_assert(pThis, tcpsrv);

	if(pThis->TCPLstnNode != NULL) {
		free(pThis->TCPLstnNode);
		pThis->TCPLstnNode = NULL;
	}

	pArg = strchr(cOptarg, ':');
	if(pArg != NULL) {
		*pArg++ = '\0';
		pThis->TCPLstnNode = cOptarg;
		cOptarg = strdup(pArg);
	}
...
static int *create_tcp_socket(tcpsrv_t *pThis)
{
	...
	dbgprintf("creating tcp socket on port %s\n", TCPLstnPort);
        ...
        error = getaddrinfo(pThis->TCPLstnNode, TCPLstnPort, &hints, &res);
        if(error) {
