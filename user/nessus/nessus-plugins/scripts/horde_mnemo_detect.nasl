...
if (description) {
  ...
  name["english"] = "Mnemo Detection";
  script_name(english:name["english"]);
 
  script_description(english:desc["english"]);
 
  summary["english"] = "Checks for presence of Mnemo";
...
foreach dir (cgi_dirs()) {
  ...
  files = make_list(
    "/horde/services/help/?module=mnemo&show=about",
  ...
  foreach file (files) {
    ...
    if (egrep(string:res, pattern:"^HTTP/.* 200 OK")) {
      ...
      #
      # - version 2.x
      if (file == "/horde/services/help/?module=mnemo&show=about") {
        pat = '>This is Mnemo +(.+).<';
      }
      # - version 1.x
...
if (installs && report_verbosity > 0) {
  if (installs == 1) {
    ...
    if (ver == "unknown") {
      info = string("An unknown version of Mnemo was detected on the remote host under\nthe path ", dir, ".");
    }
    else {
      info = string("Mnemo ", ver, " was detected on the remote host under the path\n", dir, ".");
    }
  }
  else {
    info = string(
      "Multiple instances of Mnemo were detected on the remote host:\n",
      "\n"
    );
    foreach dir (keys(installations)) {
