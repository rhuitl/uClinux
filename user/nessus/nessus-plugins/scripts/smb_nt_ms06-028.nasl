...
if(description)
{
 ...
 name["english"] = "Vulnerabilities in Microsoft PowerPoint Could Allow Remote Code Execution (916768)";
...
if ( v ) 
{
 if(ereg(pattern:"^9\..*", string:v))
 {
  ...
  if(sub != v && int(sub) < 8942 ) { security_hole(port); exit(0); }
 }
 else if(ereg(pattern:"^10\..*", string:v))
 {
  # PowerPoint XP - fixed in 10.0.6800.0
   middle =  ereg_replace(pattern:"^10\.0\.([0-9]*)\.[0-9]*$", string:v, replace:"\1");
  if(middle != v && int(middle) < 6800) { security_hole(port); exit(0); }
 }
 else if(ereg(pattern:"^11\..*", string:v))
 {
  # PowerPoint 2003 - fixed in 11.8024.0
   middle =  ereg_replace(pattern:"^11\.0\.([0-9]*)\.[0-9]*$", string:v, replace:"\1");
  if(middle != v && int(middle) < 8024) { security_hole(port); exit(0); }
 }
}
