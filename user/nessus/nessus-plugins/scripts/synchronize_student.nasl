...
The remote host is running SMART Technologies SynchronEyes Student.
This software allows teacher to remotely control student desktops 
and/or allow students to join a class.
...
if (description)
{
 ...
 script_name(english:"SynchronEyes Student detection");
 script_description(english:desc["english"]);
 script_summary(english:"Determine if a remote host is running SynchronEyes Student");
 script_category(ACT_GATHER_INFO);
 script_family(english:"Service detection");
 script_copyright(english:"This script is Copyright (C) 2006 Tenable Network Security");
 script_require_ports (5485);
 exit(0);
}
...
ret = parse_packet (data:buf, format:make_list (0,0,1,0,0,2,2,2,2,0));
if (isnull(ret))
  exit (0);
...
version = string (ret[5],".",ret[6],".",ret[7],".",ret[8]);

report = string (desc["english"],
		"\n\nPlugin output :\n\n",
		"The remote host is running SynchronEyes Student version ", version , "\n");
