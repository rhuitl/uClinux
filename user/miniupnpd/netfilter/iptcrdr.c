/* $Id: iptcrdr.c,v 1.1 2007-12-27 05:33:40 kwilson Exp $ */
/* MiniUPnP project
...
/* default chain name to use, both in the nat table
 * and the filter table */
static const char miniupnpd_chain_default[] = "MINIUPNPD";

/* chain names that can be used to override the default
 * configured using the conf file */
char *miniupnpd_preroute_chain;
char *miniupnpd_forward_chain;

#ifdef SECURE_COMPUTING
#define PREROUTE_CHAIN_NAME (miniupnpd_preroute_chain != NULL ? miniupnpd_preroute_chain : miniupnpd_chain_default)
#define FORWARD_CHAIN_NAME (miniupnpd_forward_chain != NULL ? miniupnpd_forward_chain : miniupnpd_chain_default)
#else
#define PREROUTE_CHAIN_NAME miniupnpd_chain_default
#define FORWARD_CHAIN_NAME miniupnpd_chain_default
#endif
                  ...
                  u_int64_t * packets, u_int64_t * bytes)
{
	...
	if(!iptc_is_chain(PREROUTE_CHAIN_NAME, h))
	{
		syslog(LOG_ERR, "1:chain %s not found", PREROUTE_CHAIN_NAME);
	}
	else
	{
		for(e = iptc_first_rule(PREROUTE_CHAIN_NAME, &h);
		    e;
                           ...
                           u_int64_t * packets, u_int64_t * bytes)
{
	...
	if(!iptc_is_chain(PREROUTE_CHAIN_NAME, h))
	{
		syslog(LOG_ERR, "chain %s not found", PREROUTE_CHAIN_NAME);
	}
	else
	{
		for(e = iptc_first_rule(PREROUTE_CHAIN_NAME, &h);
...
static int
delete_rule_and_commit(int is_nat_table, unsigned int index, iptc_handle_t *h,
                       const char * logcaller)
{
	int r = 0;
	if(!iptc_delete_num_entry(is_nat_table ? PREROUTE_CHAIN_NAME : FORWARD_CHAIN_NAME, index, h))
	{
...
delete_redirect_and_filter_rules(unsigned short eport, int proto)
{
	...
	if(r == 0)
	{
		...
		if(h)
		{
			r = delete_rule_and_commit(1, index, &h, "delete_redirect_rule");
		}
		h = iptc_init("filter");
		if(h && (r == 0))
		{
			r = delete_rule_and_commit(0, index, &h, "delete_filter_rule");
		}
	}
                            ...
                            const char * logcaller)
{
	...
	if(!iptc_is_chain(nat_rule ? PREROUTE_CHAIN_NAME : FORWARD_CHAIN_NAME, h))
	{
		syslog(LOG_ERR, "%s : iptc_is_chain() error : %s\n",
		       logcaller, iptc_strerror(errno));
		return -1;
	}
	if(!iptc_append_entry(nat_rule ? PREROUTE_CHAIN_NAME : FORWARD_CHAIN_NAME, e, &h))
	{
