/* $Id: minissdp.c,v 1.1 2007-12-27 05:33:40 kwilson Exp $ */
/* MiniUPnP project
							 ...
							 struct lan_addr_s * lan_addr)
{
	...
	{
		n_lan_addr--;
		if(AddMulticastMembership(s, lan_addr[n_lan_addr].addr.s_addr) < 0)
		{
			syslog(LOG_WARNING,
			       "Failed to add multicast membership for address %s", 
			       lan_addr[n_lan_addr].str );
		}
	}
                 ...
                 unsigned int lifetime)
{
	...
	while(known_service_types[i])
	{
		...
		if(n < 0)
		{
			syslog(LOG_ERR, "sendto(udp_notify): %m");
		}
		i++;
	}
}
