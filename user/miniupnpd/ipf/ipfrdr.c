/* $Id: ipfrdr.c,v 1.1 2007-12-27 05:33:40 kwilson Exp $ */
/* MiniUPnP project
                  ...
                  u_int64_t * packets, u_int64_t * bytes)
{
	...
	if ((dev = open(IPNAT_NAME, O_RDWR)) < 0) {
		syslog(LOG_ERR, "open(\"%s\"): %m", IPNAT_NAME);
		return -1;
	}
...
delete_filter_rule(const char * ifname, unsigned short eport, int proto)
{
	...
	do {
		...
		if (ioctl(dev, SIOCIPFITER, &wobj) == -1) {
