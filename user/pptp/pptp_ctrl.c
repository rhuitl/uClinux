...
struct PPTP_CONN {
    ...
    VECTOR *call;
    u_int16_t call_id; /* next call_id to try */
    void * closure;
...
static void ctrlp_rep( void * buffer, int size, int isbuff)
{
    ...
    debuglog("%s control packet type is %d '%s'\n",isbuff ? "Buffered" : "Sent", 
...
PPTP_CONN * pptp_conn_open(int inet_sock, int isclient, pptp_conn_cb callback)
{
    ...
    if (getsockname(inet_sock, (struct sockaddr *)&src, &srclen) < 0) return NULL;
    /* Allocate structure */
    ...
    conn->call_id = ntohs(src.sin_port);
    /* Create I/O buffers */
        ...
        char *phonenr)
{
    ...
    /* Assign call id */
    if (!vector_scan(conn->call, conn->call_id, PPTP_MAX_CHANNELS - 1, &i) &&
        !vector_scan(conn->call, 0, conn->call_id, &i))
    ...
    if (pptp_send_ctrl_packet(conn, &packet, sizeof(packet))) {
        ...
        conn->call_id = (i + 1) % PPTP_MAX_CHANNELS;
        return call;
...
#if 0 /* calling pptp_conn_destroy() here is buggy. */
...
static void logecho( int type)
{
    ...
    if( nlogecho > 0) {
        debuglog( "Echo Re%s received.", type == PPTP_ECHO_RQST ? "quest" :"ply");
        if( --nlogecho == 0)
            debuglog("no more Echo Reply/Request packets will be reported.");
    }
}
    ...
    switch (ntoh16(header->ctrl_type)) {
        ...
        case PPTP_START_CTRL_CONN_RQST:
        {
            ...
            struct pptp_start_ctrl_conn reply = {
                ...
                PPTP_HOSTNAME, PPTP_VENDOR };
            int idx, rc;
            debuglog("Received Start Control Connection Request");
            /* fix this packet, if necessary */
            idx = get_quirk_index();
            if (idx != -1 && pptp_fixups[idx].start_ctrl_conn) {
        ...
        case PPTP_START_CTRL_CONN_RPLY:
        {
            struct pptp_start_ctrl_conn *packet = 
                (struct pptp_start_ctrl_conn *) buffer;
            debuglog("Received Start Control Connection Reply");
        ...
        case PPTP_STOP_CTRL_CONN_RQST:
        {
            ...
            debuglog("Received Stop Control Connection Request.");
        ...
        {
            debuglog("Received Stop Control Connection Reply.");
        ...
        case PPTP_OUT_CALL_RQST:
        {
            ...
            debuglog("Received Outgoing Call Request.");
        ...
        case PPTP_OUT_CALL_RPLY:
        {
            ...
            debuglog("Received Outgoing Call Reply.");
        ...
        case PPTP_CALL_CLEAR_RQST:
        {
            ...
            debuglog("Received Call Clear Request.");
