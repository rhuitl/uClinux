...
void call_callback(PPTP_CONN *conn, PPTP_CALL *call, enum call_state state)
{
    ...
    switch(state) {
        ...
        case CALL_CLOSE_DONE:
            ...
            if (lci != NULL && vector_contains(conninfo->call_list, lci->unix_sock)) {
...
int callmgr_main(int argc, char **argv, char **envp)
{
    ...
    openlog("pptp_callmgr", LOG_PID, LOG_DAEMON);
    ...
    /* don't add a route to the server (added in pptp 1.7.2). We do this
     * ourselves in firewall-tools/vpn-up. */
