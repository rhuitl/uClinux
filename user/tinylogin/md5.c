...
static void Encode PROTO_LIST((unsigned char *, word32 *, unsigned int));
static void Decode PROTO_LIST((word32 *, unsigned char *, unsigned int));
...
unsigned int inputLen;			/* length of input block */
{
	...
	if ((context->count[0] += ((word32) inputLen << 3)) <
		((word32) inputLen << 3))
		context->count[1]++;
...
unsigned int len;
{
	...
	for (i = 0, j = 0; j < len; i++, j += 4)
		output[i] =
			((word32) input[j]) | (((word32) input[j + 1]) << 8) |
			(((word32) input[j + 2]) << 16) |
			(((word32) input[j + 3]) << 24);
}


char *sprint_md5_hash(int *hashval)
{
	int x = 0;
	char *out = NULL;
...
{
	int x[sizeof(MD5_CTX)];

	MD5Init(x);
	MD5Update(x, (char *) pw, strlen(pw));

	return sprint_md5_hash(x);
}
