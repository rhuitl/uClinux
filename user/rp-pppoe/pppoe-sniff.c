...
"$Id: pppoe-sniff.c,v 1.3 2000/09/10 01:53:32 dfs Exp $";
...
main(int argc, char *argv[])
{
	...
	fprintf(stderr, "\nGreat!  It looks like a standard PPPoE service.\nYou should not need anything special in PPPOE_EXTRA.\n");
	return 0;
    }

    fprintf(stderr, "\nOK, looks like you need something special in PPPOE_EXTRA.  Try this:\n\n");
    fprintf(stderr, "   PPPOE_EXTRA=\"");
    if (ServiceName != NULL && *ServiceName != 0) {
	fprintf(stderr, " -S '%s'", ServiceName);
    }
    if (DiscType != ETH_PPPOE_DISCOVERY || SessType != ETH_PPPOE_SESSION) {
	fprintf(stderr, " -f %x:%x", DiscType, SessType);
    }
    fprintf(stderr, "\"\n\n");
    return 0;
}
