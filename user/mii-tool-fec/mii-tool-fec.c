/*
    ...
    mii-tool-fec: monitor and control the MII for a network interface
	         only on FEC driver with CONFIG_FEC_IOCTL support enabled.

    Usage:

	mii-tool [-VvRrw] [-A media,... | -F media] [interface ...]
	((( The usage changes ...MaTed---)))

    This program is based on David Hinds's "mii-tool" program, which i based
	on Donald Becker's "mii-diag" program, which
    is more capable and verbose than this tool, but also somewhat
    harder to use. This is build strictly for the FEC Ethernet driver
	as implemented (and modified) for the uCdimm5272.

    Copyright (C) 2002 Arcturus Networks Inc.  by Ted Ma (MaTed) (mated@ArcturusNetworks.com)
...
/*
  TO BE DONE for FEC.C uC5272 driver ...MaTed---
  - mdio_read changed to do the busy wait (done)
  - ethtool not implemented
  - "-A" option won't take multiples
  - the relink and status display don't get invoked in FEC.C - it should 
    bu doesn't
*/

static char version[] =
"mii-tool-fec.c 0.9 2002/11/15  Arcturus Networks Inc.\n";
...
#endif
#include <linux/fec_ioctl.h>
#include "mii.h"


#define MII_DEBUG	1		/* the higher the number the more debug 7 is 
                             * current maximum */
...
static int mdio_read(int skfd, int location)
{
  ...
  struct fec_ioctl_data *mii = (struct fec_ioctl_data *)&ifr.ifr_data;
  ...
  {
	fprintf(stderr,
			"mdio_read: Enter: ifr.ifr_data = 0x%x (skfd-%d, location-%d)\n", 
			ifr.ifr_data);
  }	

  // set data_in value to start the get process
  mii->value_in = SIOC_MIIREG_GET;
  ...
  {
	fprintf(stderr, 
			"mdio_read: Start Waiting: ifr.ifr_data = 0x%x\n", 
			ifr.ifr_data);
  }	


  // set data_in value to retrieve the register value
  mii->value_in = SIOC_MIIREG_QUERY;
  // 
  while ((rc = ioctl(skfd, SIOCGMIIREG, &ifr)) == EAGAIN)
  {
	if (MII_DEBUG > 5)
	  fprintf(stderr, "SIOCGMIIREG on %s Waiting on %s\n", ifr.ifr_name,
			  strerror(errno));
  }
  if (rc < 0)
  {
  ...
  if (MII_DEBUG > 5)
  {
	fprintf( stderr, "SIOGMIIREG value returns %0lx\n", mii->value_out);
  }
    return mii->value_out;
}
...
/****************
 * media doesn't match return values for FEC.C
 * - didn't translate to the "standard set - left as 
 * raw registers ...MaTed---
 ****************/
...
/*****************
 * All wrong for the FEC.C ioctl ...MaTed---
 *****************/
...
int show_basic_mii(int sock, int phy_id)
{
  ...
  // added these for the special STAT2 (r17) link and duplex
  int	stat2_link_speed, stat2_link_duplex;
  struct fec_ioctl_data *mii = (struct fec_ioctl_data *)&ifr.ifr_data;

  /* just to be sure ...MaTed--- */
  mii_val[ MII_BMSR] = 0xffff;
  ...
  for (i = 0; i < ((verbose > 1) ? 32 : 8); i++)
  {
	...
	//	if (i == LXT972_STAT2)	// check for special 
	//	{
	//	  stat2_link_speed = mii->link_speed;
	//	  stat2_link_duplex = mii->duplex;
	//}
  }
  ...
  // Special Output for STAT2 register
  if (CONFIG_FEC)
  {
	// check if STAT2 register information, re-read even if verbose >1
	mii_val[LXT972_STAT2] = mdio_read(sock, LXT972_STAT2);
	stat2_link_speed = mii->link_speed;
	stat2_link_duplex = mii->duplex;
	printf ("\n **** Link Speed = [%d], Duplex = [%d] value = 0x%0x\n",
			stat2_link_speed, stat2_link_duplex, mii_val[ LXT972_STAT2]);
  }

  /* Descriptive rename. */
  /******************
   * this probably doesn't give nice names either
   * - haven't remapped the register bits ...MaTed
   * - did a fast check seems to be close though
   ******************/
