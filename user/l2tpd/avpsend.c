...
int add_hostname_avp (struct buffer *buf)
{
	...
	if (uname(&host_info) != 0)
		host_info.nodename[0] = '\0';
    add_header(buf, 0x6 + strlen(host_info.nodename), 0x7);
    strcpy ((char *) (buf->start + buf->len + sizeof(struct avp_hdr)), host_info.nodename);
    buf->len += 0x6 + strlen(host_info.nodename);
    return 0;
}
