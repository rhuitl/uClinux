...
int btTracker::_UpdatePeerList(char *buf,size_t bufsiz)
{
  ...
  for( ;bufsiz && *buf!='e'; buf += pos, bufsiz -= pos ){
    ...
    if(_IPsin(tmphost,tmpport,&addr) < 0){
      fprintf(stderr,"warn, delected invalid ip address %s.\n",tmphost);
      continue;
    }
  ...
  if( !cnt ) fprintf(stderr,"warn, peers list recved from tracker is empty.\n");
...
#define MAX_LINE_SIZ 32
  ...
  if( !q ){
    ...
    if(getsockopt(m_sock, SOL_SOCKET,SO_ERROR,&error,&n) < 0 ||
       error != 0 ){
      fprintf(stderr,"warn, recved nothing from tracker! %s\n",strerror(error));
    }
    return -1;
  }
  ...
  if ( r != 200 ){
	...
	fprintf(stderr,"warn, tracker redirect to a invalid url %s!\n", redirect);
	return -1;
      }
