...
discover_interfaces(int state) {
	...
	if ((local_family == AF_INET) && !setup_fallback) {
		...
		if (state == DISCOVER_RELAY) {
			/* Grab the first IP address of the first interface to 
			   listen on. As long as only one interface has been 
			   specified, then this will always work. */
			if (interfaces->address_count > 0) {
				memcpy (&local_address, &interfaces->addresses[0], 
	...
	omapi_object_t *h;
{
	...
	ip = (struct interface_info *)h;
      ...
      again:
	if (ip -> flags & INTERFACE_FALLBACK_RELAY) {
		if ((result =
		     receive_packet_fallback (ip, u.packbuf, sizeof u, &from, &hfrom)) < 0) {
			log_error ("receive_packet_fallback failed on %s: %m", ip -> name);
...
got_one_v6(omapi_object_t *h) {
	...
	int result;
#ifdef __uClinux__
	static char buf[65536];	/* maximum size for a UDP packet is 65536 */
#else
