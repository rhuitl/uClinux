	...
	struct hardware *hfrom;
{
	...
	/* If we received a packet on the fallback interface, and it wasn't
	   a BOOTREPLY, we just ignore it as the fallback interface should 
	   only be used for replies. Initial requests should be on the raw LPF
	   interfaces as these are bound to interfaces. */
