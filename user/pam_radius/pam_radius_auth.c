...
static void
pam_setenv(pam_handle_t *pamh, const char *name, const char *value, int valuelen)
{
  int namelen;
  char *p;

  namelen = strlen(name);
  p = malloc(namelen + 1 + valuelen + 1);
  if (!p)
    return;

  memcpy(p, name, namelen);
  p[namelen] = '=';
  memcpy(p + namelen + 1, value, valuelen);
  p[namelen + 1 + valuelen] = '\0';
  pam_putenv(pamh, p);
  free(p);
}
...
static attribute_t *
find_vendor_attribute(AUTH_HDR *response, uint32_t vendor, unsigned char type)
{
  attribute_t *attr = (attribute_t *) &response->data;
  ...
  vendor = htonl(vendor);
  while (len >= 2 && attr->length >= 2 && len >= attr->length) {
    if (attr->attribute == PW_VENDOR_SPECIFIC
        && attr->length >= 2 + sizeof(vendor)
        && memcmp(attr->data, &vendor, sizeof(vendor)) == 0) {
      attribute_t *vattr = (attribute_t *) (attr->data + sizeof(vendor));
      int vlen = attr->length - 2 - sizeof(vendor);

      while (vlen >= 2 && vattr->length >= 2 && vlen >= vattr->length) {
        if (vattr->attribute == type) {
          return vattr;
        }
        vlen -= vattr->length;
        vattr = (attribute_t *) ((char *) vattr + vattr->length);
      }
    }
    len -= attr->length;
    attr = (attribute_t *) ((char *) attr + attr->length);
...
#define PAM_FAIL_CHECK if (retval != PAM_SUCCESS) {goto error; }
  ...
  if (response->code == PW_AUTHENTICATION_ACK) {
    ...
    {
      /* Look for a sg group attribute in this packet */
      attribute_t *a_group;
      a_group = find_vendor_attribute(response, VENDOR_SECURE, PW_SG_GROUP);
      if (a_group != NULL && a_group->length > 2) {
        pam_setenv(pamh, PAM_ENV_SG_GROUP,
            (char *)a_group->data, a_group->length - 2);
      }
    }

    {
      attribute_t *a_idletime;
      a_idletime = find_attribute(response, PW_IDLE_TIMEOUT);
      if (a_idletime != NULL && a_idletime->length == 2 + 4) {
        char buf[10];
        snprintf(buf, sizeof(buf), "%d", ntohl(*(uint32_t *)a_idletime->data));
        pam_setenv(pamh, PAM_ENV_IDLE_TIMEOUT, buf, strlen(buf));
      }
    }
  } else {
