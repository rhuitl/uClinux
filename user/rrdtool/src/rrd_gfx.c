...
int           gfx_add_point  (gfx_node_t *node, 
  ...
  if (node->type == GFX_AREA) {
	...
	node->points_max++;
	node->path[node->points].code = ART_LINETO;
	node->path[node->points].x = x+LINEOFFSET;
	node->path[node->points++].y = y+LINEOFFSET;
	node->path[node->points].code = ART_END;
	node->path[node->points].x = 0;
	node->path[node->points++].y = 0;
#endif
...
int           gfx_render(gfx_canvas_t *canvas, 
  ...
  gfx_render_davidm (canvas, width, height, background, fp);
...
double gfx_get_text_width ( gfx_canvas_t *canvas,
  ...
  case IF_EPS:
  case IF_PDF:
#if defined(DISABLE_SVG) && defined(DISABLE_EPS) && defined(DISABLE_PDF)
...
#if defined(DISABLE_PNG) && defined(DISABLE_SVG) && defined(DISABLE_PDF) && defined(DISABLE_EPS)
