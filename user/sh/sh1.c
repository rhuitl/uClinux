...
*dupstate()
{
	...
	unsigned int dlen, blen;
	void *sp;

	dlen = ((unsigned int) &_end) - ((unsigned int) &__data_start);
	blen = ((unsigned int) brkaddr) - ((unsigned int) brkstrt);
	sp = malloc(dlen + blen);
	memcpy(sp, &__data_start, dlen);
	memcpy(sp+dlen, brkstrt, blen);
	return(sp);	
}
...
void *sp;
{
	...
	dlen = ((unsigned int) &_end) - ((unsigned int) &__data_start);
	blen = ((unsigned int) brkaddr) - ((unsigned int) brkstrt);
	memcpy(&__data_start, sp, dlen);
	memcpy(brkstrt, sp+dlen, blen);
	free(sp);
}
