 ...
 */
static int check_special_filename(char *pathname, char *filename, struct stat *buf)
{
	char dev[8192], *bname, typ;
	int maj, min;
#define NODE_PATTERN "@%[a-zA-Z0-9_+-],%c,%d,%d"

	if (!pathname || !filename || !buf)
		return 0;

	if (sscanf(filename, NODE_PATTERN, dev, &typ, &maj, &min) != 4)
		return 0;

	if (!S_ISREG(buf->st_mode)) {
		ERROR("Special pattern found on non-file: %s\n", filename);
		return -1;
	}
	if (buf->st_size != 0) {
		ERROR("Special pattern file with size: %s\n", filename);
		return -1;
	}
	...
	case 'p':
		...
		break;
	default:
		ERROR("Invalid special device type '%c' "
			"for file %s\n", typ, filename);
		return -1;
	}

	/* Rewrite the file and path name to the real name */
	strcpy(filename, dev);
	if ((bname = strrchr(pathname, '/')) == NULL)
		bname = pathname;
	else
		bname++;
	strcpy(bname, dev);
	...
	int (_readdir)(char *, char *, struct dir_info *))
{
	...
	while(_readdir(filename, dir_name, dir) != FALSE) {
		...
		check_special_filename(filename, dir_name, &buf);
