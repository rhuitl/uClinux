...
static int Multitech_5634_answer_phone(void)
     {
     int result;
...
static int Multitech_5634_init(void)
     {
     char buffer[VOICE_BUF_LEN];

     reset_watchdog();
     voice_modem_state = INITIALIZING;
     lprintf(L_MESG, "initializing Multitech 5634 voice modem");
...
static int Multitech_5634_set_compression(int *compression, int *speed,
 int *bits)
     {
     char buffer[VOICE_BUF_LEN];
...
static int Multitech_5634_set_device(int device)
     {
     reset_watchdog();
...
void Multitech_5634_fix_modem(int expect_error)
{
	char buffer[VOICE_BUF_LEN];
	int result = VMA_FAIL;
...
static int Multitech_5634_switch_to_data_fax(char *mode)
{
	char buffer[VOICE_BUF_LEN];
	...
	Multitech_5634_fix_modem(1);

	return OK;
}

static int Multitech_5634_voice_mode_off(void)
{
	char buffer[VOICE_BUF_LEN];
...
static int Multitech_5634_voice_mode_on(void)
{
	char buffer[VOICE_BUF_LEN];
...
voice_modem_struct Multitech_5634ZBAV =
     {
     "Multitech 5634ZBAV",
     "Multitech5634",
     ...
     &IS_101_play_file,
     &IS_101_record_file,
     &Multitech_5634_set_compression,
     &Multitech_5634_set_device,
     &IS_101_stop_dialing,
     &IS_101_stop_playing,
     &IS_101_stop_recording,
     &IS_101_stop_waiting,
     &Multitech_5634_switch_to_data_fax,
